...
questions that decide whether it can:
  ...
  pvt      -- constant-gm means gm(ref) = 1/R, so the CURRENT moves over
              ...
              xhigh_po poly resistor (sigma 2.5% + tempco) sets the true floor.
  integ    -- drop the reference into miller_ota in place of the ideal source
    ...
    python tb/biasgen.py op | startup | pvt | integ
"""
import math
import sys
...
def bench_op(rval=RVAL, tag_extra=""):
    net = f"""* biasgen op, rval={rval}
{header()}
{sub()}
vdd vdd 0 dc {VDD}
vss vss 0 0
xb vbias vdd 0 biasgen rval={rval}
...
def write(op, start, start_no, pvt, integ):
...
def main():
    ...
    op = start = start_no = pvt = integ = None
    if what in ("op", "all"):
        op = bench_op()
    ...
    if what in ("integ", "all"):
        ...
        print(f"integ: vout ideal={i['vout']:.4f} biasgen={b['vout']:.4f}  "
              ...
              flush=True)
    write(op, start, start_no, pvt, integ)

...
instead of two corners.
    ...
    python tb/corners.py fix      # the co-designed THD fix over PVT (§11)
    python tb/corners.py          # pvt + mc
...
# The THD fix (docs/thd.md `fix` search, design-notes.md §11): the ×1
# compensation could not hold once the output stage was scaled for THD, so
# output current AND compensation were co-designed. Rz 20k -> 10k is the
# lever (it cuts the feedforward that was pushing the UGF up); pout 2.5 is
# the output current that brings THD to 0.167 %. This re-runs THAT point over
# PVT -- a compensation change is only real once its worst corner is known.
FIX = dict(name="THD fix (×2.5, Cc 4p, Rz 10k)",
           params="pcc=4e-12 prz=10000", pout=2.5)
...
def run_fix():
    """The co-designed THD fix over PVT. Same bench_ac/bench_op as everywhere
    else, ENV mutated -- but pout is threaded into BOTH so the operating point
    is the scaled output stage, not the shipped one."""
    rows = []
    p_out = FIX["pout"]
    acp = f"{FIX['params']} pout={p_out}"
    for (p, t, v) in pvt_points():
        at(p, t, v)
        tag = f"_fix_{p}_{t}_{v}".replace(".", "p")
        ac = bench_ac("miller_ota", LOAD, params=acp, tag_extra=tag)
        op = bench_op("miller_ota", LOAD, tag_extra=tag, params=f"pout={p_out}")
        if not op.get("converged", True):
            bad = ["RUN FAILED (no op-point) — rerun this corner"]
        else:
            bad = [i for i, d in op["devices"].items()
        ...
        rows.append(dict(process=p, temp=t, vdd=v, a_lf_db=ac.get("a_lf_db"),
                         ...
                         vout=op.get("vout"), isupply=op.get("isupply"),
                         out_of_sat=bad))
        print(f"  {p:2s} {t:+4d}C {v:.2f}V  "
              f"A={ac.get('a_lf_db') or float('nan'):5.1f}dB  "
              f"UGF={(ac.get('ugf_hz') or 0)/1e6:6.2f}MHz  "
              f"PM={ac.get('pm_deg') or float('nan'):6.1f}deg  "
...
def write(pvt, mc, fix=None):
    ...
    if fix:
        pms = [r["pm_deg"] for r in fix if r["pm_deg"] is not None]
        ugfs = [r["ugf_hz"] for r in fix if r["ugf_hz"] is not None]
        iqs = [r["isupply"] for r in fix if r["isupply"] is not None]
        worst_pm = min(pms) if pms else None
        worst_ugf = min(ugfs) if ugfs else None
        worst_iq = max(iqs) if iqs else None
        wr = min((r for r in fix if r["pm_deg"] is not None),
                 key=lambda r: r["pm_deg"], default=None)
        ok = (worst_pm is not None and worst_pm >= PM_TARGET and
              worst_ugf is not None and worst_ugf >= UGF_TARGET)
        where = (f" ({wr['process']}/{wr['temp']:+d} °C/{wr['vdd']} V)"
                 if wr else "")
        L.append(f"## The THD fix over PVT — {FIX['name']}\n")
        L.append("The co-designed THD fix (`design-notes.md` §12) re-run "
                 "across the same box, pout threaded into both benches. "
                 f"**Worst-corner PM {g(worst_pm)}°**{where}, worst UGF "
                 f"{g(worst_ugf and worst_ugf/1e6, '.2f')} MHz, worst-case "
                 f"I_q {g(worst_iq and worst_iq*1e6, '.0f')} µA against the "
                 f"200 µA budget — "
                 f"{'**meets PM and UGF at every corner**' if ok else '**FAILS at some corner**'}. "
                 "(THD itself is quoted nominal in `thd.md`; corner-THD is the "
                 "one step past this.)\n")
        L.append("| process | temp | Vdd | gain @1 Hz | UGF | PM | "
                 "V_out | I_supply | devices out of saturation |")
        L.append("|---|---|---|---|---|---|---|---|---|")
...
def main():
    ...
    if what in ("mc", "both"):
        ...
        cache.write_text(json.dumps(data, indent=1))
    if what == "fix":
        print("=== THD fix over PVT ===", flush=True)
        data["fix"] = run_fix()
        cache.write_text(json.dumps(data, indent=1))
    write(data.get("pvt"), data.get("mc"), data.get("fix"))
